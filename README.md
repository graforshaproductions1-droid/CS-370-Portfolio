...
My approach to problem solving begins with understanding the requirements, identifying constraints, and breaking the larger task into smaller parts. For this project, that meant separating the environment, the agent, the neural network, and the training process. I evaluated progress through win rate, loss values, and completion checks rather than relying on appearance or assumption. That process reinforced the importance of testing, refinement, and evidence-based decision making in computer science.

Ethical responsibility remains essential when building intelligent systems. Developers must consider reliability, fairness, transparency, privacy, and the possible effects of a system on its users and organization. A model that performs well technically can still create harm if it is used carelessly or without proper safeguards. This project reinforced that responsible computer science requires both technical competence and careful judgment.
